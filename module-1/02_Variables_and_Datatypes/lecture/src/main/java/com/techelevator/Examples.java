...
public class Examples {
	...
	public static void main(String[] args) {
		...
		long myLong = 123;
		int myInt = 3;
		//myInt = myLong;  // causes a compiler error because the long variable is capable of holding values that won't fit in an int
		myLong = myInt;
		...
		double averageCookiesEaten = cookiesEaten / numberOfChildrenEatingCookies; // the right hand side of this operation is evaluated as an int, so the fractional value is truncated
		System.out.println("Average Cookies Eaten: " + averageCookiesEaten);
		
		
		averageCookiesEaten = (double)cookiesEaten / numberOfChildrenEatingCookies; // here we "cast" an int variable to type double so that the result of the arithmetic operation is a double
		System.out.println("(casting to double) Average Cookies Eaten: " + averageCookiesEaten);
